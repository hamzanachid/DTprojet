...
public class NoteDaoImpl implements NoteDao {
    ...
    public Note getById(Long id) {
        ...
        try (Connection conn = connectionManager.getConnection();
            ...
            throw new RuntimeException("Error retrieving note", e);
        }
        return null;
    }
    ...
    public List<Note> getByEtudiant(Long etudiantId) {
        ...
        try (Connection conn = connectionManager.getConnection();
            ...
            throw new RuntimeException("Error retrieving notes by student", e);
        }
        return notes;
    }
    ...
    @Override
    public double calculateElementAverage(Long elementId) {
        String sql = "SELECT AVG(note) FROM note n JOIN modalite_evaluation m ON n.modalite_evaluation_id = m.id WHERE m.element_module_id = ?";
        try (Connection conn = connectionManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setLong(1, elementId);
            ResultSet rs = stmt.executeQuery();
            ...
            throw new RuntimeException("Error calculating element average", e);
        }
        return 0.0;
    }

    @Override
    public double calculateModuleAverage(Long moduleId) {
        String sql = "SELECT AVG(n.note) FROM note n " +
                "JOIN modalite_evaluation m ON n.modalite_evaluation_id = m.id " +
                "JOIN element_de_module e ON m.element_module_id = e.id " +
                "WHERE e.module_id = ? AND e.est_valide = true";
        try (Connection conn = connectionManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            throw new RuntimeException("Error calculating module average", e);
        }
        return 0.0;
    }

    @Override
    public void setDraft(Long noteId, boolean isDraft) {
        String sql = "UPDATE note SET is_draft = ? WHERE id = ?";
        try (Connection conn = connectionManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            throw new RuntimeException("Error setting draft status", e);
        }
    }
